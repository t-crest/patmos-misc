...
# BB#0:                                      # %entry
	...
	{        pmov	$p4 =  $p2               #l1: whether to execute the loop
			 pmov	$p6 =  $p3 }             #l2: whether to execute the loop
.LBB0_1:                                     # %for.cond
...
.LBB0_3:                                     # %for.cond2
	...
	{ ( $p4) cmplt	$p1 = $r3, $r2           #l1: 99<i <=> !(i<100)
	  ( $p6) cmplt	$p1 = $r3, $r2 }         #l2: 299<i <=> !(i<300)
	{        pand	$p5 =  $p4, !$p1         #l1: p4 && i<100
			 pand	$p7 =  $p6, !$p1 }       #l2: p6 && i<300
	{ ( $p5) lwc	$r6 = [$r4]              #l1: load _2
	  ( $p6) pmov	$p6 = !$p1 }             #l2: p6 = i<300
	{ ( $p7) lwc	$r6 = [$r4]              #l2: load _3
	  ( $p4) pmov	$p4 = !$p1 }             #l1: p4 = i<100
	  ( $p5) add	$r6 = $r5, $r6           #l1: i_2 + _2
	{ ( $p4) br	.LBB0_1                      #l1: loop if i<100 
	  ( $p7) sub	$r6 = $r5, $r6 }         #l2: i_2 - _3
	{ ( $p6) br	.LBB0_3                      #l2: loop if i<300
	  ( $p5) add	$r1 = $r1, $r6 }         #l1: x += i + _2 (executes before branch)
	{ ( $p7) add	$r1 = $r1, $r6           #l2: x += i - _3 (executes before branch)
	  ( $p5) add	$r5 = $r5, 1 }           #l1: i_2++       (executes before branch)
	  ( $p7) add	$r5 = $r5, 1             #l2: i_2++       (executes before branch)
                                # %if.end
